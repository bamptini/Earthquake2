...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        JSONArray quakeArray = quake.getJSONArray("features");

            for (int i=0 ; i < quakeArray.length(); i++) {
                JSONObject currentData = quakeArray.getJSONObject(i);
                JSONObject properties = currentData.getJSONObject("properties");

                //Extract the values for the key called "mag"
                double magnitude = properties.getDouble("mag");
                //Extract the values for the key called "place"
                String location = properties.getString("place");
                //Extract the values for the key called "time"
                long eventdate = properties.getLong("time");
                //Extract the values for the key called "url"
                String url = properties.getString("url");

                //Convert time in milliseconds into a Date object by calling the date constructor
                long timeInMilliseconds = eventdate;
                Date dateObject = new Date(timeInMilliseconds);

                //Initialise a SimpleDateFormat instance to make is more readable
                SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                String dateToDisplay = dateFormatter.format(dateObject);

                // Create a new {@link Earthquake} object with the magnitude, location, time,
                // and url from the JSON response.
                Earthquake earthquake = new Earthquake(magnitude, location, eventdate, url);
